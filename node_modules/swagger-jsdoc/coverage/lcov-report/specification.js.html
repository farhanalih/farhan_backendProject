...
<head>
    <title>Code coverage report for specification.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> specification.js</h1>
        <div class='clearfix'>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>85/102</span>
            </div>
...
<span class="cline-any cline-yes">7x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import doctrine from 'doctrine';
...
<span class="cstat-no" title="statement not covered" >      specification = api;</span>
    }
  });
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (specification.openapi) {
...
export function organize(swaggerObject, annotations) {
  for (const annotation of annotations) {
    for (const property in annotation) {
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (property === 'x-webhooks') {
<span class="cstat-no" title="statement not covered" >        swaggerObject[property] = annotation[property];</span>
      }
&nbsp;
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (property.startsWith('x-')) <span class="cstat-no" title="statement not covered" >continue;</span>
&nbsp;
...
&nbsp;
      ...
      } else <span class="missing-if-branch" title="else path not taken" >E</span>if (property === 'tags') {
...
<span class="cstat-no" title="statement not covered" >        swaggerObject.paths[property] = mergeDeep(</span>
...
&nbsp;
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (anchors.length) {
<span class="cstat-no" title="statement not covered" >          for (const anchor of anchors) {</span>
<span class="cstat-no" title="statement not covered" >            yamlDocsAnchors.set(anchor, parsed);</span>
          }
        } else <span class="missing-if-branch" title="else path not taken" >E</span>if (parsed.errors &amp;&amp; parsed.errors.length) {
          yamlDocsErrors.push(parsed);
        } else {
<span class="cstat-no" title="statement not covered" >          yamlDocsReady.push(parsed);</span>
        }
      }
    }
&nbsp;
...
&nbsp;
      docWithErr.errors.forEach((error, index) =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (error.name === 'YAMLReferenceError') {
          ...
          const refErr = <span class="cstat-no" title="statement not covered" >error.message</span>
            .split('Aliased anchor not found: ')
            .filter(<span class="fstat-no" title="function not covered" >(a</span>) =&gt; <span class="cstat-no" title="statement not covered" >a)</span>
            .join('')
            .split(' at line')[0];
&nbsp;
          const anchor = <span class="cstat-no" title="statement not covered" >yamlDocsAnchors.get(refErr);</span>
          const anchorString = <span class="cstat-no" title="statement not covered" >anchor.cstNode.toString();</span>
          const originalString = <span class="cstat-no" title="statement not covered" >docWithErr.cstNode.toString();</span>
          const readyDocument = <span class="cstat-no" title="statement not covered" >YAML.parseDocument(</span>
            `${anchorString}\n${originalString}`
          );
&nbsp;
<span class="cstat-no" title="statement not covered" >          yamlDocsReady.push(readyDocument);</span>
<span class="cstat-no" title="statement not covered" >          errsToDelete.push(index);</span>
        }
      });
      // reverse sort the deletion array so we always delete from the end
      errsToDelete.sort(<span class="fstat-no" title="function not covered" >(a</span>, b) =&gt; <span class="cstat-no" title="statement not covered" >b - a)</span>;
&nbsp;
      // Cleanup solved errors in order to allow for parser to pass through.
      for (const errIndex of errsToDelete) {
<span class="cstat-no" title="statement not covered" >        docWithErr.errors.splice(errIndex, 1);</span>
      }
    }
&nbsp;
    const errReport = yamlDocsErrors
      ...
      .filter((error) =&gt; !!error);
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (errReport.length) {
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Sun Mar 07 2021 12:25:48 GMT+0100 (Central European Standard Time)
            </div>
        </div>
